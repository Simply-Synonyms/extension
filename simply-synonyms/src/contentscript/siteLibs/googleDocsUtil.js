// Google Docs Util
// Derived from https://github.com/JensPLarsen/ChromeExtension-GoogleDocsUtil/
    ...
    for (var i = 0; i < paragraphrenderers.length; i++) {
      ...
      for (var j = 0; j < lineviews.length; j++) {
        ...
        for (var k = 0; k < wordhtmlgeneratorWordNodes.length; k++) {
          var wordhtmlgeneratorWordNodeRect =
            wordhtmlgeneratorWordNodes[k].getBoundingClientRect()
          if (caretRect) {
  ...
  function highlight(startIndex, endIndex, googleDocument) {
    for (var i = 0; i < googleDocument.nodes.length; i++) {
      ...
      if (
        ...
        var parentRect =
          googleDocument.nodes[i].lineElement.getBoundingClientRect()
